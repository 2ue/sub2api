...
## 2026-05-03 尺寸透传与计费分档纠偏

本次对 OpenAI 图片 `size` 处理做了明确纠偏：本地服务不再因为未知尺寸或疑似不满足 OpenAI 官方约束的尺寸返回 400。请求中的 `size` 保持原样透传给上游，是否接受该尺寸由上游官方接口或兼容提供商决定。本地只在成功产出最终图片后使用 `size` 计算计费档位。

官方文档依据：`gpt-image-2` 支持满足约束的任意 `size`，常用尺寸包括 `1024x1024`、`1536x1024`、`1024x1536`、`2048x2048`、`2048x1152`、`3840x2160`、`2160x3840`、`auto`；超过 `2560x1440` 总像素的输出属于更高风险/实验区间。因此本项目采用如下计费分档：
...
1. 不改写 `size`，不把未知尺寸改成预设尺寸。
2. 不在本地实现官方尺寸合法性校验。
3. 不因未知尺寸产生本地 400；无效参数错误必须来自上游响应。
4. 只在成功产图后扣费，失败请求不按图片计费。
5. 计费分档与生成透传使用同一个解析逻辑，避免生成允许但计费报错。
