...
- 后端测试覆盖：补充图片生成访问控制、计费、并发、流式断连、多行 SSE、usage fallback 和模型 alias 的测试。
- 部署配置示例：暴露图片生成并发隔离、图片流式超时、图片 keepalive 和图片计费相关配置项。
- 回归验证：已完成 image 相关后端回归测试，覆盖 `internal/config`、`internal/handler`、`internal/service`、`internal/server`。
